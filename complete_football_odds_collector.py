...
class CompleteFootballOddsCollector:
    ...
    Sauvegarde dans data/odds/ pour organisation cohérente
    """
    ...
    def __init__(self, rapidapi_key: str):
        ...
        # MODIFICATION: Organisation dans data/odds/ au lieu de complete_odds_data/
        self.odds_folder = os.path.join("data", "odds")
    ...
    def load_existing_fixtures(self, league_code: str) -> List[int]:
        """
        Charge les IDs des matchs existants depuis les fichiers CSV des matchs dans data/
        Avec vérification intelligente des données manquantes
        """
        # MODIFICATION: Chercher dans data/ (niveau principal) pour les fichiers de matchs
        data_folder = "data"
    ...
    def collect_complete_league_odds(self, league_code: str) -> pd.DataFrame:
        ...
        logger.info(f"   📁 Destination: {self.raw_folder}")
    ...
    def save_complete_odds(self, df: pd.DataFrame, league_code: str) -> None:
        """
        Sauvegarde complète avec fichiers multiples et résumés dans data/odds/
        """
        if df.empty:
    ...
    def run_complete_odds_collection(self) -> None:
        ...
        logger.info(f"📁 Destination: {self.odds_folder}")
    ...
    def save_global_metadata(self) -> None:
        """
        Sauvegarde les métadonnées globales de la collecte dans data/odds/
        """
        try:
            metadata = {
                'collection_info': {
                    ...
                    'destination_folder': self.odds_folder
                },
                'api_statistics': {
                ...
                'folder_structure': {
                    'raw_data': self.raw_folder,
                    'summaries': self.summary_folder,
                    'metadata': self.odds_folder
                }
            }
            
            # Sauvegarde en JSON dans data/odds/
...
def main():
    ...
    Lance la collecte complète des odds avec organisation dans data/odds/
    """
    import os
    ...
    logger.info("📁 Organisation des données: data/odds/")
    ...
    # Affichage final de l'organisation des fichiers
    logger.info("\n📂 STRUCTURE FINALE DES DONNÉES:")
    logger.info("data/")
    logger.info("├── [fichiers matchs existants].csv")
    logger.info("└── odds/")
    logger.info("    ├── raw_data/")
    logger.info("    │   └── [ligue]_complete_odds.csv")
    logger.info("    ├── summaries/")
    logger.info("    │   ├── [ligue]_bookmaker_summary.csv")
    logger.info("    │   └── [ligue]_bet_types_summary.csv")
    logger.info("    └── collection_metadata.json")
    logger.info("\n🎯 Collecte terminée - Données organisées dans data/odds/")
